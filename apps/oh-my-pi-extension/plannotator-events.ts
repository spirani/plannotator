...
const events = createPlannotatorEvents<ExtensionContext>({
	reviewStatusPath: join(getPlannotatorDataDir(), "oh-my-pi-review-status.json"),
	browserActions: {
	...
	getRecentAssistantMessages,
	getStartupErrorMessage,
	resolveAnnotationFolder: () => undefined,
	resolvePiModels: (ctx, options) =>
		typeof options.piModels === "function" ? options.piModels(ctx) : options.piModels,
	resolvePiExecutablePath: (_ctx, options) => options.piExecutablePath,
});
