...
const REVIEW_STATUS_PATH = join(getPlannotatorDataDir(), "oh-my-pi-review-status.json");
...
export interface PlannotatorBrowserActions {
	startPlanReviewBrowserSession: typeof startPlanReviewBrowserSession;
	openCodeReview: typeof openCodeReview;
	openMarkdownAnnotation: typeof openMarkdownAnnotation;
	openLastMessageAnnotation: typeof openLastMessageAnnotation;
	openArchiveBrowserAction: typeof openArchiveBrowserAction;
}
...
export interface PlannotatorEventListenerOptions {
	...
	piModels?: PlannotatorModel[] | ((ctx: ExtensionContext) => PlannotatorModel[]);
	/** Executable used to spawn the embedding host for Ask AI. */
	piExecutablePath?: string;
	/** Hermetic test seam; production uses the lazy browser wrappers above. */
	browserActions?: Partial<PlannotatorBrowserActions>;
}
...
export function registerPlannotatorEventListeners(
	...
	const actions: PlannotatorBrowserActions = {
	...
	const resolvePiModels = (ctx: ExtensionContext): PlannotatorModel[] | undefined =>
