	...
	const agentJobs = createAgentJobHandler({
		...
		async buildCommand(provider, config) {
			...
			if (provider === "guide") {
				...
				// The shared guide factory correctly generates Pi marker prompts but
				// necessarily targets Pi's binary. Oh My Pi owns this host's model
				// credentials, so preserve the generated prompt and replace only its
				// executable/flags before spawning the Pi guide or a Pi guide repair.
				if (!built) return built;
				const command =
					built.engine === "pi" &&
					built.prompt &&
					isOhMyPiExecutable(options.piExecutablePath)
						? buildOhMyPiMarkerCommand(
								built.prompt,
			...
			if (markerEngine) {
				...
				const usesOhMyPi =
					markerEngine.id === "pi" && isOhMyPiExecutable(options.piExecutablePath);
				const { command } = usesOhMyPi
					? buildOhMyPiMarkerCommand(prompt, model, thinking, options.piExecutablePath)
