...
import { contentHash, deleteDraft } from "../../draft.ts";
import { loadConfig, saveConfig, detectGitUser, getServerConfig, resolveAIEnabled, resolveSharingEnabled, resolveCursorSandbox, resolveGuideHistory } from "../../config.ts";
...
import { resolvePoolCwd, type WorktreePool } from "../../worktree-pool.ts";
import { createCommitAvatarResolver } from "../../commit-avatars.ts";
...
import { type AgentJobInfo, REVIEW_OUTPUT_FAILED, getAgentJobAnnotationContext, markJobReviewFailed } from "../../agent-jobs.ts";
...
import { resolveOpenInTarget } from "../../html-assets-node.ts";
import {
	fetchPR,
	fetchPRContext,
	fetchPRFileContent,
...
} from "../../codex-review.ts";
import { buildAgentReviewUserMessage, buildAgentReviewUserMessageForTarget, type WorkspaceReviewPromptContext } from "../../agent-review-message.ts";
import {
...
import { createTourSession, TOUR_EMPTY_OUTPUT_ERROR } from "../../tour-review.ts";
import { createGuideSession, GUIDE_EMPTY_OUTPUT_ERROR } from "../../guide-review.ts";
import { createGuideStoreSession, SAVED_GUIDE_ID_PREFIX } from "../../guide-store.ts";
import {
	MARKER_ENGINES,
...
import type { SemanticDiffAvailability, SemanticDiffResponse } from "../../semantic-diff-types.ts";
import { discoverCuratedSkills, resolveRequestedReviewProfile, listAllSkills, enableReviewSkill } from "../../review-skill-loader.ts";
import {
...
/**
 * Host-owned Pi command construction. The common runtime stays agnostic to
 * embedding-agent executable flags while retaining its standard Pi command.
 */
export interface PiMarkerCommandPolicy {
	matchesExecutable(binary: string | undefined): boolean;
	build(prompt: string, model?: string, thinking?: string, binary?: string): MarkerCommandResult;
}
...
export async function startReviewServer(options: {
	...
	agentCwd?: string;
	/**
	 * Host-provided model catalog for an embedded Pi-compatible agent.
	 * Supplying this also makes the catalog authoritative for the Pi marker
	 * engine, so the UI and spawned review jobs use the same model scope.
	 */
	piModels?: Array<{ id: string; label: string; default?: boolean }>;
	/** Pi-compatible executable override (for example, Oh My Pi's `omp`). */
	piExecutablePath?: string;
	/** Host override for Pi marker commands (for example, Oh My Pi's `omp`). */
	piMarkerCommandPolicy?: PiMarkerCommandPolicy;
	...
	let prListCache: import("../../pr-types.ts").PRListItem[] | null = null;
	...
	const prStackTreeCache = new Map<string, import("../../pr-types.ts").PRStackTree | null>();
	...
	let prStackTree: import("../../pr-types.ts").PRStackTree | null = null;
	if (prRef && prMeta) {
	...
	const agentJobs = createAgentJobHandler({
		...
		...(options.piModels !== undefined && { markerModels: { pi: options.piModels } }),
		...(options.piExecutablePath !== undefined && { markerBinaries: { pi: options.piExecutablePath } }),
		...
		async buildCommand(provider, config) {
			...
			if (provider === "guide") {
				...
				if (repairOf) {
					...
					const failedEngine =
						typeof config?.engine === "string" && config.engine ? config.engine : undefined;
					const failedEngineBinary =
						failedEngine === "pi" && options.piExecutablePath
				...
				// The shared guide factory always uses Pi's standard command. Hosts
				// with a compatible executable can replace just the argv while
				// retaining the generated prompt for both guide and repair jobs.
				if (!built) return built;
				const command =
					built.engine === "pi" &&
					built.prompt &&
					options.piMarkerCommandPolicy?.matchesExecutable(options.piExecutablePath)
						? options.piMarkerCommandPolicy.build(
								built.prompt,
								built.model || undefined,
								built.thinking,
								options.piExecutablePath,
							).command
						: built.command;
				if (!command) return built;
				const guidedRun = { ...built, command };
			...
			if (markerEngine) {
				...
				const usesHostPiMarkerCommand =
					markerEngine.id === "pi" &&
					options.piMarkerCommandPolicy?.matchesExecutable(options.piExecutablePath);
				const { command } = usesHostPiMarkerCommand
					? options.piMarkerCommandPolicy!.build(prompt, model, thinking, options.piExecutablePath)
					: buildMarkerCommand(markerEngine, prompt, model, cwd, {
							thinking,
							cursorSandbox: resolveCursorSandbox(loadConfig()),
						});
	...
	const aiRuntime = aiEnabled
		? await createPiAIRuntime({
				getCwd: resolveAgentCwd,
				models: options.piModels,
