...
import { isAIEndpointPath } from "../../ai/endpoints.ts";
import { resolveCommandFromWhichOutput } from "../../ai/providers/command-path.ts";
...
interface CreatePiAIRuntimeOptions {
	...
	models?: ReadonlyArray<{ id: string; label: string; default?: boolean }>;
...
export async function createPiAIRuntime(options: CreatePiAIRuntimeOptions = {}): Promise<PiAIRuntime | null> {
	try {
		...
		try {
			await import("../../ai/providers/claude-agent-sdk.ts");
		...
		try {
			await import("../../ai/providers/codex-app-server.ts");
		...
		try {
			await import("../../ai/providers/pi-sdk-node.ts");
			const piCommand = options.piExecutablePath ?? "pi";
			const piPath = whichCmd(piCommand);
			if (piPath) {
				const provider = await ai.createProvider({
					type: "pi-sdk",
					cwd,
					piExecutablePath: piPath,
				} as Parameters<typeof ai.createProvider>[0]);
				if (options.models !== undefined) {
					Object.assign(provider, { models: options.models });
				}
				if (provider && "fetchModels" in provider && options.models === undefined) {
					modelDiscovery.push(
			...
			// Pi-compatible agent not available.
		}

		try {
			await import("../../ai/providers/opencode-sdk.ts");
