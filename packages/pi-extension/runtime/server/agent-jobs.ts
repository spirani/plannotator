...
import { formatClaudeLogEvent } from "../../claude-review.ts";
import {
	MARKER_ENGINES,
...
export interface AgentJobHandlerOptions {
	...
	getCwd: () => string;
	/**
	 * Host-provided marker model catalogs. An entry is authoritative, including
	 * an empty list, so embedded agents do not trigger a second discovery path.
	 */
	markerModels?: Partial<Record<MarkerEngineId, MarkerModel[]>>;
	/** Executable overrides for embedded marker engines. */
	markerBinaries?: Partial<Record<MarkerEngineId, string>>;
 ...
 */
async function discoverMarkerModels(engine: MarkerEngine, binary: string): Promise<MarkerModel[]> {
	try {
		const { stdout } = await execFileAsync(binary, engine.modelsArgv, {
			timeout: 5000,
			encoding: "utf8",
		});
...
export function createAgentJobHandler(options: AgentJobHandlerOptions) {
	...
	const markerModels = options.markerModels ?? {};
	const markerBinaries = options.markerBinaries ?? {};
	const markerBinary = (engine: MarkerEngine): string =>
		markerBinaries[engine.id] ?? engine.binary;
	const markerAvailable = (engine: MarkerEngine): boolean =>
		mode === "review" && whichCmd(markerBinary(engine));
	...
	const capabilities: AgentCapability[] = [
		...
		{ id: "tour", name: "Code Tour", available: whichCmd("claude") || whichCmd("codex") },
		{
			...
			available:
				whichCmd("claude") ||
				whichCmd("codex") ||
				(mode === "review" && Object.values(MARKER_ENGINES).some(markerAvailable)),
		},
	];
	...
	for (const engine of Object.values(MARKER_ENGINES)) {
		capabilities.push({
			id: engine.id,
			name: engine.name,
			available: markerAvailable(engine),
		});
	}
	const markerModelsCache = new Map<string, MarkerModel[]>();
	for (const [engineId, models] of Object.entries(markerModels)) {
		if (models !== undefined) markerModelsCache.set(engineId, models);
	}
	...
	async function buildCapabilitiesResponse(): Promise<AgentCapabilities> {
		const providers = await Promise.all(capabilities.map(async (c) => {
			...
			if (models === undefined) {
				models = await discoverMarkerModels(engine, markerBinary(engine));
